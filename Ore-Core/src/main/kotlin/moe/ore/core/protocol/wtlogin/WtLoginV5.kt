...
import moe.ore.core.net.packet.writeOicqRequestPacket
...
class WtLoginV5(var uin: Long) {
    fun build(seq: Int): ByteArray {
        val manager = DataManager.manager(uin)
        val tlv = Tlv(uin)
        return buildUniPacket(manager, bodyType = 2, commandName = "wtlogin.exchange_emp", key = ByteArray(16)) {
//        writeSsoPacket(client, client.subAppId, WtLogin.ExchangeEmp.commandName, sequenceId = sequenceId) {
            writeOicqRequestPacket(dataManager = manager, encryptType = 69, 0x0810, manager.wLoginSigInfo.wtSessionTicketKey) {
...
//            }
                ...
                writeBytes(tlv.t116())
//        bytePacketBuilder.t116(client.miscBitMap, client.subSigMap)
                ...
                writeBytes(tlv.t100())
//        bytePacketBuilder.t100(appId, 2, client.appClientVersion, client.ssoVersion, client.mainSigMap)
                writeBytes(tlv.t107())
//        bytePacketBuilder.t107(0)

                //t108(client.ksid) // 第一次 exchange 没有 108
                writeBytes(tlv.t144())
                writeBytes(tlv.t142())
                writeBytes(tlv.t145())
//        bytePacketBuilder.t144(client)
//        bytePacketBuilder.t142(client.apkId)
//        bytePacketBuilder.t145(client.device.guid)

//        val noPicSig = client.wLoginSigInfo.noPicSig
//            ?: error("Internal error: doing exchange emp 15 while noPicSig=null")
//        bytePacketBuilder.t16a(noPicSig)
...
//        bytePacketBuilder.t154(0)
//        bytePacketBuilder.t141(client.device.simInfo, client.networkType, client.device.apn)
//        bytePacketBuilder.t8(2052)
//        bytePacketBuilder.t511()
//        bytePacketBuilder.t147(appId, client.apkVersionName, client.apkSignatureMd5)
//        bytePacketBuilder.t177(buildTime = client.buildTime, buildVersion = client.sdkVersion)

                // new
//        bytePacketBuilder.t400(g = client.G, uin = client.uin, guid = client.device.guid, dpwd = client.dpwd, appId = 1, subAppId = 16, randomSeed = client.randSeed)
                ...
                writeBytes(tlv.t202())
                writeBytes(tlv.t516())
//        bytePacketBuilder.t187(client.device.macAddress)
//        bytePacketBuilder.t188(client.device.androidId)
//        bytePacketBuilder.t194(client.device.imsiMd5)
//        bytePacketBuilder.t202(client.device.wifiBSSID, client.device.wifiSSID)
//        bytePacketBuilder.t516()

                writeBytes(tlv.t521())
                writeBytes(tlv.t525())
//        bytePacketBuilder.t521() // new
//        bytePacketBuilder.t525(client.loginExtraData) // new
                //t544() // new
