 ...
 * @constructor
 */
class LoginHelper(private val uin: Long, private val client: BotClient, private val listener: OreListener?) : Thread() {
    ...
    private fun succ(decrypt: ByteArray) {
        ...
        manager.wLoginSigInfo.tgtKey = tlvMap119.getOrDefault(0x10d, ByteArray(0))
        println(manager.wLoginSigInfo.tgtKey.toHexString())
    }

    private fun onDevLockLogin(tlvMap: Map<Int, ByteArray>) {
        manager.wLoginSigInfo.t104 = tlvMap.getOrElse(0x104) { TODO("error") }

        // client.dpwd = getRandomString(16).toByteArray()
        manager.wLoginSigInfo.sigInfo2 = (device.guid + manager.wLoginSigInfo.dpwd + tlvMap.getOrElse(0x402) { TODO() }).md5()

        handle(WtLoginV2(uin).sendTo(client))
        // println("403： " + tlvMap[0x403]?.toUHexString())
//        return LoginPacketResponse.DeviceLockLogin(bot)
    }

    private fun onSolveLoginCaptcha(tlvMap: Map<Int, ByteArray>) {
        /*
        ...
        manager.wLoginSigInfo.t104 = tlvMap.getOrElse(0x104) { TODO() }
        tlvMap[0x192]?.let {
...
//            return tlvMap.get(0x105).toReadPacket().withUse {
//                val imageData = this
