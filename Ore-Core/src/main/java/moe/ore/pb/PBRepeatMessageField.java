...
public final class PBRepeatMessageField<T extends MessageMicro<T>> extends PBField<List<T>> {
	...
	protected int computeSizeDirectly(int i, List<T> list) {
		...
		while (true) {
			...
			T messageMicro = it.next();
			j = messageMicro.computeSizeDirectly(i, messageMicro) + j;
		}
	}

	protected void copyFrom(PBField<List<T>> pBField) {
		PBRepeatMessageField<T> field = (PBRepeatMessageField<T>) pBField;
		if (field.isEmpty()) {
			this.value = Collections.emptyList();
			return;
		}
		List<T> list = get();
		Class<?> cls = field.get(0).getClass();
		int size = field.value.size() - list.size();
		if (size > 0) {
			for (int i = 0; i < size; i++) {
				try {
					list.add(DebugUtil.forcedConvert(null, cls.newInstance()));
		...
		for (int i = 0; i < list.size(); i++) {
			list.get(i).copyFrom(field.value.get(i));
		}
	}
	...
	public void readFrom(CodedInputStreamMicro codedInputStreamMicro) {
		try {
			MessageMicro<T> messageMicro = this.helper.newInstance();
			codedInputStreamMicro.readMessage(messageMicro);
			add(DebugUtil.forcedConvert(null, messageMicro));
		} catch (InstantiationException | IllegalAccessException e) {
			e.printStackTrace();
		}
	}
